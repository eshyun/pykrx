...
def krx_login(*args, **kwargs):
    return krx.login(*args, **kwargs)

def enable_auto_login_on_failure(
    enabled: bool = True, *, allow_dup_login: bool = False
):
    return krx.enable_auto_login_on_failure(enabled, allow_dup_login=allow_dup_login)


def clear_session_file():
    return krx.clear_session_file()


_INDEX_FALLBACK_TICKER_NAME = {
    "1001": "코스피",
    "2001": "코스닥",
}


_INDEX_FALLBACK_MARKET_TICKERS = {
    "KOSPI": ["1001"],
    "KOSDAQ": ["2001"],
    "KRX": ["1001", "2001"],
    "테마": [],
...
_INDEX_FALLBACK_NAVER_SYMBOL = {
    "1001": "KOSPI",
    "2001": "KOSDAQ",
}


def _get_index_ohlcv_by_date_fallback(
    fromdate: str, todate: str, ticker: str
) -> DataFrame:
    symbol = _INDEX_FALLBACK_NAVER_SYMBOL.get(str(ticker))
    if symbol is None:
        return DataFrame()
    df = naver.get_index_ohlcv_by_date(fromdate, todate, symbol)
    return df


def _get_index_price_change_by_ticker_fallback(
    ...
    rows = []
    for t in market2tickers.get(market, []):
        df = _get_index_ohlcv_by_date_fallback(fromdate, todate, t)
        if df.empty:
            continue
        open_ = float(df["시가"].iloc[0])
        close_ = float(df["종가"].iloc[-1])
        vol = int(df["거래량"].sum()) if "거래량" in df.columns else 0
        val = int(df["거래대금"].sum()) if "거래대금" in df.columns else 0
        rate = (close_ - open_) / open_ * 100 if open_ != 0 else 0.0
        rows.append(
            {
                "지수명": get_index_ticker_name(t, fallback=True),
                "시가": open_,
                "종가": close_,
                "등락률": rate,
                "거래량": vol,
    ...
    if not rows:
        return DataFrame()
    out = DataFrame(rows).set_index("지수명")
    return out[["시가", "종가", "등락률", "거래량", "거래대금"]]


def _get_index_fundamental_by_date_fallback(
    fromdate: str, todate: str, ticker: str
) -> DataFrame:
    df = _get_index_ohlcv_by_date_fallback(fromdate, todate, ticker)
    if df.empty:
        return DataFrame()

    close = df["종가"].astype(float)
    prev_close = close.shift(1)
    fluc = (close - prev_close) / prev_close * 100
    fluc = fluc.replace([np.inf, -np.inf], 0).fillna(0)
...
def _get_index_listing_date_fallback(계열구분: str) -> DataFrame:
    rows = []
    if 계열구분 in ("KOSPI", "KRX"):
        rows.extend(
            [
                {
                    "지수명": "코스피",
...
@deprecated(
    version="1.1",
    reason="You should use get_market_net_purchases_of_equities" "_by_ticker() instead",
)
...
def get_index_ticker_list(
    date: str = None, market: str = "KOSPI", fallback: bool = True
) -> list:
    """모든 지수 (index)의 티커 조회
    ...
    try:
        ...
        if not fallback:
            raise
        return _INDEX_FALLBACK_MARKET_TICKERS.get(market, [])


def get_index_ticker_name(ticker: str, fallback: bool = True) -> str:
    """티커의 이름 조회
    ...
    try:
        ...
        if not fallback:
            raise
        if ticker in _INDEX_FALLBACK_TICKER_NAME:
            return _INDEX_FALLBACK_TICKER_NAME[ticker]
        raise


def get_index_portfolio_deposit_file(
    ticker: str, date: str = None, alternative: bool = False, fallback: bool = True
) -> list:
    """지수 구성 종목 조회
...
def get_index_ohlcv_by_ticker(
    date, market: str = "KOSPI", alternative: bool = False, fallback: bool = True
):
    """티커별로 정리된 전종목 OHLCV
    ...
    try:
        ...
        t = market2ticker.get(market)
        if t is None:
            return DataFrame()

        # Naver는 개별 지수만 제공하므로 대표 지수 1개 row만 반환
        d1 = naver.get_index_ohlcv_by_date(date, date, _INDEX_FALLBACK_NAVER_SYMBOL[t])
        if d1.empty:
            return DataFrame()
        row = d1.iloc[-1]
        out = DataFrame(
        ...
        return out[["시가", "고가", "저가", "종가", "거래량", "거래대금"]]
    holiday = (df[["시가", "고가", "저가", "종가"]] == 0).all(axis=None)
    if holiday and alternative:
        target_date = get_nearest_business_day_in_a_week(date=date, prev=True)
        try:
...
def get_index_ohlcv_by_date(
    ...
    if name_display:
        df.columns.name = get_index_ticker_name(ticker, fallback=fallback)
...
def get_index_fundamental_by_ticker(
    date: str, market: str = "KOSPI", alternative: bool = False, fallback: bool = True
) -> DataFrame:
    """특정 일자의 전종목 PER/PBR/배당수익률 조회
    ...
    try:
        ...
        if not fallback:
            raise
        # Naver 기반으로 대표 지수만 종가/등락률 채우고 나머지는 0
        market2ticker = {
            "KOSPI": "1001",
            "KOSDAQ": "2001",
        }
        ...
        if t is None:
            return DataFrame()
        d1 = _get_index_fundamental_by_date_fallback(date, date, t)
        if d1.empty:
            return DataFrame()
        row = d1.iloc[-1]
        out = DataFrame(
        ...
        return out[["종가", "등락률", "PER", "선행PER", "PBR", "배당수익률"]]
...
def get_index_fundamental_by_date(
    fromdate: str, todate: str, ticker: str, prev: bool = True, fallback: bool = True
) -> DataFrame:
    """특정 종목의 지정된 기간 PER/PBR/배당수익률 조회
    ...
    try:
        ...
        if not fallback:
            raise
        return _get_index_fundamental_by_date_fallback(fromdate, todate, ticker)


def get_index_listing_date(계열구분: str = "KOSPI", fallback: bool = True) -> DataFrame:
    """[11004] 전체지수 기본정보
    ...
    try:
        ...
        if not fallback:
            raise
        return _get_index_listing_date_fallback(계열구분)
...
def get_index_price_change(
    fromdate: str, todate: str, market: str = "KOSPI", fallback: bool = True
) -> DataFrame:
    """입력된 기간동안의 전체 지수 등락률
    ...
    return get_index_price_change_by_ticker(fromdate, todate, market, fallback=fallback)
...
def get_index_price_change_by_ticker(
    ...
    try:
        ...
        if not fallback:
            raise
        return _get_index_price_change_by_ticker_fallback(fromdate, todate, market)
